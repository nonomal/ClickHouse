...
struct IcebergMetadataFilesCacheCell : private boost::noncopyable
{
    ...
    /// - manifest list consists of cache keys which will retrieve the manifest file from cache (ManifestFileCacheKeys)
    /// - manifest file (ManifestFilePtr)
    std::variant<String, LatestMetadataVersionPtr, /*MetadataVersion,*/ ManifestFileCacheKeys, Iceberg::ManifestFilePtr> cached_element;
    Int64 memory_bytes;
