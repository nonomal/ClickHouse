...
#define LIST_OF_SERVER_SETTINGS_WITHOUT_PATH(DECLARE, ALIAS) \
    ...
    **See also:**
    ...
    DECLARE(UInt64, iceberg_metadata_async_refresher_pool_size, DEFAULT_ICEBERG_METADATA_ASYNC_REFRESHER_POOL_SIZE, "Size of thread pool to asynchronously fetch the latest metadata from a remote iceberg catalog; the pool is shared by all the active tables.", 0) \
